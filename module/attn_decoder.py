...
Source: https://github.com/ayumiymk/DiG/blob/main/models/attn_decoder.py
LastEditTime: 2023-07-24 21:04:24
'''
...
class AttentionRecognitionHead(nn.Module):
  ...
  def __init__(self, num_classes, in_planes, sDim, attDim, max_len_labels):
    ...
    # self.register_buffer("state", torch.zeros(1, out_dim))
    # self.register_buffer("padded_outputs", torch.zeros(1, out_dim))
    # self.register_buffer("y_prev", torch.zeros(1, out_dim))
  ...
  def forward_train(self, x):
    ...
    padded_outputs = torch.zeros(batch_size, self.max_len_labels, self.num_classes).to(x.device)
    for i in range(max(lengths)):
      if i == 0:
        y_prev = torch.zeros((batch_size)).fill_(self.num_classes).to(x.device) # the last one is used as the <BOS>.
      else:
        y_prev = targets[:,i-1]
    ...
    padded_outputs[:, :max(lengths), :] = outputs
  ...
  def sample(self, x):
    ...
    for i in range(self.max_len_labels):
      if i == 0:
        y_prev = torch.zeros((batch_size)).fill_(self.num_classes).to(x.device)
      else:
        y_prev = predicted
